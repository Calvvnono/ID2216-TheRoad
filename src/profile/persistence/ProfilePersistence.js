...
export const ProfilePersistence = {
  ...
  async loadAll() {
    ...
    try {
      ...
      const signinAward = isDailyAwardSatisfied(
        ...
        ? { isAwarded: false }
        : await this.awardDailySigninXp();
      const latestProfile = signinAward?.isAwarded
        ? await this.fetchProfile()
        : profile;

      profileStore.setProfileData(latestProfile, wishlist, preferences);
    } catch (e) {
      profileStore.setLoadError(e.message ?? 'Failed to load profile');
    }
  },
  ...
  async refreshWishlist() {
    try {
      ...
      profileStore.setWishlist(wishlist);
    } catch {
  ...
  async openWishlistPlaceDetail(item) {
    profileStore.setWishlistDetailLoading({
      id: item.id,
      name: item.name,
    ...
    const detail = await this.fetchPlaceDetail(item.id, item.name);
    profileStore.setWishlistPlaceDetail(detail);
  },
  ...
  async updatePreferences(newPrefs) {
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(newPrefs, 'budgetPerDay')) {
        await this.awardBudgetSavedXp();
      }
      const refreshedProfile = await this.fetchProfile();
      profileStore.setPreferencesSaved(newPrefs, refreshedProfile);
      return true;
    } catch (e) {
      profileStore.setError(e.message ?? 'Failed to save preferences');
      return false;
    }
  },

  async uploadAvatar(localUri) {
    profileStore.setAvatarUploadStarted();
    try {
      const nextProfile = await this.saveAvatar(localUri);
      profileStore.setAvatarUploadSuccess(nextProfile);
    } catch (e) {
      profileStore.setAvatarUploadError(e.message ?? 'Failed to upload avatar');
    }
  },
  ...
  async pickAndUploadAvatar() {
    ...
    this.uploadAvatar(result.assets[0].uri);
  },

  async exportData() {
    profileStore.setExportStarted();
    ...
    try {
      ...
      profileStore.setExportSuccess(refreshedProfile);
    } catch (e) {
      profileStore.setExportError(e.message ?? 'Export failed');
    }
  },
